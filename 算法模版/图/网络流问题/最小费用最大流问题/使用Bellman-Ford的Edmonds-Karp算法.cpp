void insertEdge(LL from,LL to,LL cap,LL cost){//插入边
    graph[from].push_back(edges.size());
    edges.push_back(Edge(from,to,cap,0,cost));
    graph[to].push_back(edges.size());
    edges.push_back(Edge(to,from,0,0,-cost));
}
bool BellmanFord(LL s,LL t,LL&flow,long long&cost){//最大流算法,s为源点,t为汇点
    LL a[MAXV]={0},p[MAXV]={0},dis[MAXV]={0};//a数组表示源点到结点a[i]的残量,p数组表示最短路树上到达结点p[i]的边在edges数组中的序号
