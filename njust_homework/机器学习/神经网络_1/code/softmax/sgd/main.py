...
def gen_ans(dataX, dataY, ALPHA, LAMBDA, acceptCost, maxTimes):
    ...
    # thetas = np.random.rand(classNum, dataFeatures)  # 随机初始化权重矩阵
    thetas = np.zeros((classNum, dataFeatures))  # 初始化权重矩阵000
    ...
    for i in range(maxTimes):
        ...
        indexs = [random.randint(0, len(classYHot) - 1) for i in range(0, 1)]  # 用其中一个进行更新
        # 计算梯度
        gradient = -1.0 * np.dot((classYHot[indexs] - hThetaX[indexs]).T, dataX[indexs]) + LAMBDA * thetas
...
if __name__ == '__main__':
    ...
    plt.show()
    plotX2 = -(thetas[0, 0] + thetas[0, 1] * oriDataX[:, 1]) / thetas[0, 2]
    # 作出原始值
